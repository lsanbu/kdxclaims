...
if not SUPABASE_URL or not SUPABASE_KEY:
    raise RuntimeError("Missing SUPABASE_URL or SUPABASE_KEY. Check your .env and current working directory.")
...
def upload_claim(claim: ClaimIn):
    ...
    if claim.claim_type == "fuel":
        ...
        ack = (
            f"✅ Driver Salary claim saved\n"
            f"Date: {claim.claim_date}\n"
            f"Payee: {claim.station or 'Driver'}\n"
            f"Ref: {claim.reference_no}\n"
            f"Amount: ₹{claim.total_rs}"
        )
...
def send_tg(chat_id: int, text: str):
    ...
    _ = requests.post(url, json={"chat_id": chat_id, "text": text})
